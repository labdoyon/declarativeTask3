...
from xpd_to_tsv_config import removeCards, matrixSize, matrixTemplate, classPictures, presentationCard, feedback_time
...
def extract_matrix_and_data(i_folder, i_file, recognition=False, learning=False, association=False):
    ...
    if learning or not recognition:
        ...
        for index, category in enumerate(classPictures):
            ...
            cards_order = header2[cards_order_index + 1: cards_order_index + 5]
...
def write_csv(output_file, matrix_pictures,
    ...
    first_row = ['Item', 'Class', 'Sound', 'Position']
    if not days:
